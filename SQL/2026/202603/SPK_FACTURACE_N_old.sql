...
-- ----------------------------------------------------  
-- ULTIMOS CAMBIOS:  
-- 1.dic.2025 feature: Cobro de Moderadoras en AUTD y Pagos compartidos en Citas.  
-- 23.ago.2025 fix: variable @CA es varchar(1), estaba llenandose con 'Administradora' y 'Afiliado'  
  ...
  If @CA='A' --'Afiliado'  
   ...
   BEGIN  
    ...
    SELECT @FTR_IDTRANSACCION, @CNSFTR, @CNSFTR, GETDATE(), 'DB', CIT.IDAREA, NULL, VR_TOTAL=0,   
     ...
     VLR_SERVICI=CIT.VALORTOTAL, VLR_COPAGOS=CIT.VALORCOPAGO, VLR_PAGCOMP=CIT.VALORPCOMP, VALORMODERADORA=CIT.VALORMODERADORA, CIT.IDMEDICO, @NOAUT, @NOAUT, 1,   
   ...
   END  
    ...
    from @Tabla_N_FACTURA  
     ...
     /* 2025.12.01: Ya no se calcula por acá porque las facturas de Copagos/PComp/Moderadora se hacen por factura financiera y el proceso las vincula  
     ...
     */  
     ...
     If @EC<>1                          Exec dbo.SPK_FAC_IMPDEDUC @NIT, @CNSFTR, @N_FACTURA, @VRSERV;  
