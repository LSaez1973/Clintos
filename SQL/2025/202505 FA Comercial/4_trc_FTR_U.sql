 ...
 begin try          
  ...
  begin  
   ...
   from HADMF a join inserted b on a.N_FACTURA=b.N_FACTURA and b.ESTADO='A';
	...
	-- Facturación masiva con FMAS:
	-- Asigna -A a los consecutivos de admisiones, sirve para destruir relación y evitar errores en algunos reportes relacionados
	-- Se podría considerar mas adelante eliminar los registros en vez de actualizar su admisión.
	update d set NOADMISION = d.NOADMISION+'-A'
	from FMAS a with(nolock) 
		join inserted b on b.N_FACTURA = a.N_FACTURA and b.ESTADO = 'A'
		join FMASD d on d.CNSFMAS = a.CNSFMAS
	where not d.NOADMISION like '%-A';
  end  
 end try  
 begin catch  
