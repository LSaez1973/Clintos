...
as
	select ORIGEN='HADM', b.IDTERCEROCA, b.COBRARA, a.IDSERVICIOADM, h.IDSEDE, a.NOADMISION, h.FECHAALTA, h.IDAREA_ALTA, h.CCOSTO_ALTA, 
		...
		VALORCOPAGO =		cast(IIF(b.TIPOCOPAGO='7', coalesce(b.VALORCOPAGO,0),		0) as decimal(14,2)),
		VALORMODERADORA =	cast(IIF(b.TIPOCOPAGO='8', coalesce(b.VALORMODERADORA,0),	0) as decimal(14,2)),
		VALORPCOMP =		cast(IIF(b.TIPOCOPAGO='9', coalesce(b.VALORPCOMP,0),		0) as decimal(14,2)),
...
as
	...
	where h.CERRADA=1 and b.FACTURABLE=1 and coalesce(h.CLASENOPROC,'')<>'NP' -- and a.ESTADO='Atendido' 
		and 1=2 -- No se facturan, las OM se deben cargar por prestaci n, osea que se facturan por HADM
go
...
as
	select ORIGEN='AUT', a.IDTERCEROCA, a.COBRARA, a.IDSERVICIOADM, a.IDSEDE, a.NOADMISION, FECHAALTA=a.FECHA, IDAREA_ALTA=a.IDAREA, CCOSTO_ALTA=a.CCOSTO, 
		...
		VALORMODERADORA =	cast(IIF(b.TIPOCOPAGO='8', coalesce(b.VALORMODERADORA,0),	0) as decimal(14,2)),
		VALORPCOMP =		cast(IIF(b.TIPOCOPAGO='9', coalesce(b.VALORPCOMP,0),		0) as decimal(14,2)),
		DESCUENTO=coalesce(a.DESCUENTO,0),b.PCOSTO, b.FACTURADA, b.N_FACTURA, a.IDPROVEEDOR,
...
as  
	select ORIGEN='CIT', a.IDTERCEROCA, a.COBRARA, a.IDSERVICIOADM, a.IDSEDE, a.NOADMISION, FECHAALTA=a.FECHA, IDAREA_ALTA=a.IDAREA, CCOSTO_ALTA=a.CCOSTO,   
		...
		CANTIDAD=1,VALORTOTAL=a.VALORTOTAL*g.C,VLR_SERVICI=a.VALORTOTAL*g.C, 
		VALORCOPAGO =		cast(IIF(a.TIPOCOPAGO='7', coalesce(a.VALORCOPAGO,0),		0) as decimal(14,2)),
		VALORMODERADORA =	cast(IIF(a.TIPOCOPAGO='8', coalesce(a.VALORMODERADORA,0),	0) as decimal(14,2)),
		VALORPCOMP =		cast(IIF(a.TIPOCOPAGO='9', coalesce(a.VALORPCOMP,0),		0) as decimal(14,2)),
		DESCUENTO=cast(coalesce(a.DESCUENTO,0) as decimal(14,2)),   
		PCOSTO=cast(coalesce(a.VALORTOTALCOS,0) as decimal(14,2)), a.FACTURADA, a.N_FACTURA, IDPROVEEDOR=a.IDMEDICO,  
		a.IDAREA,a.CCOSTO,c.IDCUM,f.NOINVIMA,KCNTRID=cast(0 as int),d.NUMCONTRATO,a.KNEGID,a.IDTARIFA,d.KCNTID, IDSERVICIOREL=Null, 
		AFIRCID=cast(coalesce(a.AFIRCID,0) as int), a.CNSFACT, a.MARCAFAC,  
	...
	from dbo.cit a With (NoLock)   
		...
		left join dbo.HADM h with(nolock) on h.NOADMISION = a.NOADMISION and h.CONSECUTIVOCIT = a.CONSECUTIVO
	where a.FACTURABLE=1 and Not a.FECHALLEGA Is null 
		and h.NOADMISION is null
go
...
as
	...
	/* No se facturan, las OM se deben cargar por prestaci n, osea que se facturan por HADM
	union all 
...
go
-- -----------------------------------------  
-- 20251127: Add VALORMODERADORA
CREATE VIEW [dbo].[VW_PRE_HADM]    
AS    
	SELECT FMAS.CNSFMAS, HPRE.NOADMISION, HPRE.NOPRESTACION, PRE.PREFIJO, HPRED.IDSERVICIO, HPRED.VALOR, HPRED.CANTIDAD,vt.VALORTOTAL, 
		VALORCOPAGO = coalesce(HPRED.VALORCOPAGO,0), /*+ cop.COPAGO_DIST*/ 
		VALORPCOMP = coalesce(HPRED.VALORPCOMP,0), 
		VALORMODERADORA = coalesce(HPRED.VALORMODERADORA,0),
		ve.VALOREXCEDENTE, HPRED.IDCIRUGIA, HPRED.TIPOSERCIRUGIA,
