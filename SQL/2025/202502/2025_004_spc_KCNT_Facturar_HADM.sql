...
begin
	...
	with biva as (
		...
		From FTR f with(nolock)
			cross apply (
				select d.PIVA, BASEIVA=sum(d.VLR_SERVICI), VIVA=sum(d.VIVA), ITEMS=count(*)
				From FTRD d with(nolock) 
				where d.CNSFTR=f.CNSFCT and d.PIVA>0 group by d.PIVA
			) b
			cross apply (select TIVA=cast(b.PIVA*b.BASEIVA*0.01 as decimal(14,2))) i
	...
	with biva as (
		select f.N_FACTURA, f.CNSFCT, b.*, i.nVIVA
		From FTR f with(nolock)
			cross apply (
				select d.PIVA, BASEIVA=sum(d.VLR_SERVICI), VIVA=sum(d.VIVA), ITEMS=count(*)
				From FTRD d with(nolock) 
			...
			cross apply (select nVIVA=round(b.PIVA*b.BASEIVA*0.01,2)) i
		where f.N_FACTURA=@N_FACTURA
	)	
	update f set VIVA=b.nVIVA, VR_TOTAL=nt.nVLR_TOTAL
	--select f.VR_TOTAL, nt.nVLR_TOTAL, Diff=f.VR_TOTAL - nt.nVLR_TOTAL, f.VIVA, b.nVIVA, deiffIVA= f.VIVA-b.nVIVA  
	from ftr f with(nolock)
		join biva b on b.CNSFCT=f.CNSFCT
		cross apply (select nVLR_SERVICI=sum(VLR_SERVICI) from ftrd d with(nolock) where d.CNSFTR=f.CNSFCT) d
		cross apply (select nVLR_TOTAL= d.nVLR_SERVICI + b.nVIVA - f.VALORCOPAGO - f.VALORMODERADORA - f.VALORPCOMP - f.DESCUENTO) nt;
			...
			begin
				...
				begin
					...
					with v as (
						...
						from #Datos_I a
							cross join #ftrofr f
						where coalesce(a.VALOR,0)*coalesce(a.CANTIDAD,0) > 0
						group by f.COPAGOS
					)
					update #Datos_I set 
...
with mem1 as (
	...
	WHERE a.NOADMISION=@NOADMISION and coalesce(a.CIRUGIA,'NO')='NO'
...
begin   
	...
	if update (VALOR) or update(CANTIDAD) or update(VALORCOPAGO) or update(VALORPCOMP) 
	begin
		with mem1 as (
			SELECT a.NOPRESTACION, VT=SUM(b.VALOR * b.CANTIDAD), VE = SUM(b.VALOREXCEDENTE), VC = SUM(b.VALORCOPAGO), VP = SUM(b.VALORPCOMP)
			FROM hpre a with (nolock)
...
drop procedure if exists spc_FTR_DistribuirCopagos
go
create procedure spc_FTR_DistribuirCopagos  
	@N_FACTURA_COPAGOS varchar(20)
as
begin
	set nocount on;
	...
	declare 
		@TOTALEXCEDENTE decimal(14,2), @string varchar(128), @ESTADO varchar(1), @PROCEDENCIA varchar(20), @TranCounter int;;

	if coalesce(@N_FACTURA_COPAGOS,'')=''
		return;
	...
	begin try 
		...
		-- Asignar Estado Generada
		insert into @FTR_GENERADA(CNSFCT,N_FACTURA,NOADMISION,IDT,TIPOFAC,COPAGOS,ORIGENINGASIS,ESTADO) -- Faturas Generadas
		select CNSFCT,N_FACTURA,NOREFERENCIA as NOADMISION,IDT,TIPOFAC,VR_TOTAL as COPAGOS,ORIGENINGASIS,ESTADO
		from ftr where N_FACTURA=@N_FACTURA_COPAGOS; 	

		--select * from @FTR_GENERADA;
		select @PROCEDENCIA=ORIGENINGASIS from @FTR_GENERADA;
		...
		-- select sum(VALORCOPAGO) from #Datos_I;
	...
	-- select sum(VALORCOPAGO) as VALORCOPAGO, sum(VALORPCOMP) as VALORPCOMP from HPRED where N_FACTURACOPAGO=@N_FACTURA_COPAGOS
end	
go
