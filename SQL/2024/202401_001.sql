...
go
alter table FMASD drop column IDAFINONIMO;
...
select top 10 * from hpred 
select distinct tipocopago from hpred
...
select NOMCATEGORIA,PORCENTAJE=cast(PORCENTAJE as decimal(14,2)) from fnc_PorcentajesIMP('IVA',getdate()) order by PORCENTAJE desc

select PORCENTAJE=round(PORCENTAJE,2) from fnc_PorcentajesIMP('IVA',getdate()) where IDCATEGORIA='03'
select NOMCATEGORIA,PORCENTAJE=cast(PORCENTAJE*100 as int) from fnc_PorcentajesIMP('IVA','22/11/2024') order by PORCENTAJE desc

select IDGRUPOIMP,* from ser where iva='Si'

print 21.18 *100

Select * From dbo.fnc_KCNT_unServicio('02','900156264','EPSS',' 30654162','09','01','874124C','25/10/2024',2,1)


select * from sysobjects where name like '%IVA%' and xtype='fn'
select * from IMPD
select * from IMPDV where IDIMPUESTO='IVA'


update SER set  IVA='Si', IDIMPUESTO='IVA', IDCLASE='01' where idservicio='MX0000634PBS'


FNK_CALCULA_IVA
sp_helptext tra_HPRED_I
select * from fna_verTriggers('HPRED')
